                                ...
                                <tbody>
                                    @foreach ($types as $type)
                                        <tr class="d-flex">
                                            ...
                                            <td class="col-3">
                                                <div>
                                                    <a href="{{ route('payment.show', $type->id) }}" class="btn btn-outline-info btn-flat">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a href="{{ route('payment.edit', $type->id) }}" class="btn btn-outline-warning btn-flat">
                                                        <i class="fas fa-pen"></i>
                                                    </a>
                                                    <form action="{{ route('payment.delete', $type->id) }}" id="deleteForm{{ $type->id }}" method="post" hidden>
                                                        @csrf
                                                        @method('delete')
                                                    </form>
