                        ...
                        <form class="form-horizontal" action="{{ route('status.update', $status->id) }}" method="post">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group row">
                                    ...
                                    <input type="text" class="form-control col" id="title" name="title" value="{{ $status->title }}" placeholder="Новое наименование">
                                </div>
                            </div>

                            <div class="card-footer">
                                <button type="submit" class="btn btn-outline-success">Подтвердить</button>
                                <a type="button" href="{{ route('status.index') }}" class="btn btn-outline-danger float-right">Отменить</a>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="card card-secondary">
                        ...
                        <div class="card-body">
                            <dl class="row">
                                ...
                                <dd class="col-lg-9">{{ $status->id }}</dd>
                                <dt class="col-lg-3">Наименование:</dt>
                                <dd class="col-lg-9">{{ $status->title }}</dd>
                                <dt class="col-lg-3">Создан:</dt>
                                <dd class="col-lg-9">{{ $status->created_at }}</dd>
                                <dt class="col-lg-3">Изменён:</dt>
                                <dd class="col-lg-9">{{ $status->updated_at }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
